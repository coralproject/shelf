...
func Midware(mongoURI *url.URL) web.Middleware {
	...
	m := func(next web.Handler) web.Handler {
		...
		h := func(c *web.Context) error {
			...
			if err != nil {
				log.Error(c.SessionID, "Midware", err, "Method[%s] URL[%s] RADDR[%s]", c.Request.Method, c.Request.URL.Path, c.Request.RemoteAddr)
			...
			log.Dev(c.SessionID, "Midware", "Capture Mongo Session")

			// Close the MongoDB session when the handler returns.
			defer func() {
				log.Dev(c.SessionID, "Midware", "Release Mongo Session")
